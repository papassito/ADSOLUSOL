...
function Read-LinesSafe {
    param([string]$Path)
    if ($script:FileLinesCache.ContainsKey($Path)) {
...
$jsonFiles = @($allFiles | Where-Object Extension -eq ".json") # Corregido: ahora los archivos .json se incluyen correctamente
...
$servicesToVerify = @("CoreSignatureVerifier","MarketingBrainClient","CampaignService")
$usedServices = [System.Collections.Generic.Dictionary[string, bool]]::new()
foreach ($s in $servicesToVerify) { $usedServices[$s] = $false }
...
foreach ($f in $csFiles) {
    ...
    foreach ($s in $servicesStillToFind) {
        if ($fileContent -match [regex]::Escape($s)) {
            $usedServices[$s] = $true
        }
    }
}
...
foreach ($ep in $expectedApiPatterns) {
    $found = $false
    # Optimización: buscar patrón por archivo sin cargar todo a memoria
    foreach ($f in $apiFiles) {
        $lines = Read-LinesSafe $f.FullName
        if (($lines -join "`n") -match $ep.Pattern) {
            Add-Finding -Severity "INFO" -Code "ADS-API-PRESENT" -Category "API" `
                -Message ("Endpoint/ruta localizada: " + $ep.Name) `
                -File (Get-RelativePathSafe $f.FullName)
            $found = $true
            break
        }
    }
...
$filesForIntegrationCheck = @($csFiles + $mdFiles)
$integrationChecks = @(
...
foreach ($c in $integrationChecks) {
    $found = $false
    foreach ($f in $filesForIntegrationCheck) {
        # Con el cache de archivos, esta operación es de bajo costo
        if (((Read-LinesSafe $f.FullName) -join "`n") -match $c.Regex) {
            $found = $true
            break
        }
    }
