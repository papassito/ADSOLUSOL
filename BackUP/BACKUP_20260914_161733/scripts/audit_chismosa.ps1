...
if (-not $dotnetPath) {
    $portableDotnet = 'C:\Users\Radio 2027\Documents\Codex\tools\dotnet\dotnet.exe'
    if (Test-Path -LiteralPath $portableDotnet) { $dotnetPath = $portableDotnet }
}
if ($dotnetPath) {
...
$csFiles = Get-ChildItem -Path (Join-Path $root 'src') -Filter "*.cs" -Recurse | 
